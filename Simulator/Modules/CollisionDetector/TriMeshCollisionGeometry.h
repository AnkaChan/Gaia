...
namespace GAIA {
    ...
    inline embree::Vec3fa loadVertexPos(const TriMeshFEM* pMesh, int32_t vId)
    {
        return embree::Vec3fa::loadu(pMesh->vertex(vId).data());
    }

    embree::Vec3fa faceNormal(const TriMeshFEM* pMesh, int32_t faceId);
    embree::Vec3fa faceOrientedArea(const TriMeshFEM* pMesh, int32_t faceId);
    ...
    inline embree::Vec3fa faceOrientedArea(const TriMeshFEM* pMesh, int32_t faceId)
    {
