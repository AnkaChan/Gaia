...
#ifdef GPU_JACOBI_DX
		ManagedBuffer<FloatingTypeGPU>::SharedPtr dxBuffer;
#endif // GPU_JACOBI_DX
...
#ifdef GPU_JACOBI_DX
		dxBuffer = std::make_shared<ManagedBuffer<FloatingTypeGPU>>(pTetMesh->inertia.size(), false);
		SET_GPU_MESH_BUFER_PTR(pTetMeshGPU, dx);
#endif // GPU_JACOBI_DX
