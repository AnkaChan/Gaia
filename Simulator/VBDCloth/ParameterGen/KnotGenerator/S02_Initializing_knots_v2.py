...
if __name__ == '__main__':
    ...
    resolution = (numberOfKnots* 600, 20)
    outFileName = f"knot_V2_{resolution[0]}x{resolution[1]}"
    ...
    circleCenter = [head[0][0], -0.3, 0.035]

    for iCirles in range(1, nCircles + 1):
        for iDiv in range(0, circleDivisions):
            if iDiv ==0 and iCirles == 1:
                continue
            head.append(
                [circleCenter[0] + (iCirles + iDiv / circleDivisions) * circleStep,
                 circleCenter[1] + circleRadius * np.cos(2 * np.pi * iDiv / circleDivisions),
                 circleCenter[2] + circleRadius * np.sin(2 * np.pi * iDiv / circleDivisions)]
            )
        # head.append(
